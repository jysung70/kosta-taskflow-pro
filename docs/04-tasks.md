...
| 4 | `models.py` — Task 모델 7필드 정의 및 테이블 생성 | ⬜ | `tasks.db` 파일 생성, `sqlite3 tasks.db ".tables"` 에서 `tasks` 테이블 확인 |
| 5 | `schemas.py` — Pydantic 스키마 (Create·Update·Response) 작성 | ⬜ | `python -c "from schemas import TaskCreate; print('OK')"` 오류 없음 |
| 6 | `POST /api/tasks` 구현 — 201 반환, title 필수 검증 | ⬜ | `curl -X POST` 로 201 확인, title 없이 요청 시 400 확인 |
| 7 | `GET /api/tasks` 구현 — 200 반환, description 제외 | ⬜ | curl 응답 JSON에 `description` 필드 없음 확인 |
...
| 10 | `DELETE /api/tasks/{id}` 구현 — 204 반환, Swagger UI 전체 확인 | ⬜ | `http://localhost:8000/docs` 에서 5개 엔드포인트 모두 표시 및 실행 확인 |
...
| # | 작업 | 상태 | 검증 방법 |
|---|---|---|---|
| 1 | `frontend/` 폴더 구조 생성 (`index.html`, `app.js`, `style.css`) | ⬜ | 3개 파일 존재 확인, 브라우저에서 빈 화면 오류 없이 열림 확인 |
...
| 4 | 업무 추가 폼 구현 (`title`, `status`, `due_at`) — `POST /api/tasks` 연결 | ⬜ | 폼 제출 후 201 응답, title 미입력 시 브라우저단 오류 메시지 표시 확인 |
...
| 8 | 3초 폴링 연결, 360px 반응형 최종 점검, 성공 기준 5개 통과 후 `git push` | ⬜ | `01-product.md` 성공 기준 5개 항목 체크 후 전체 push 확인 |
